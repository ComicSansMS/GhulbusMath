#ifndef INCLUDE_GUARD_GHULBUS_LIBRARY_MATH_MATRIX4_HPP
#define INCLUDE_GUARD_GHULBUS_LIBRARY_MATH_MATRIX4_HPP
...
public:
    ...
    constexpr Matrix4()
        :m11{}, m12{}, m13{}, m14{},
         ...
         m41{}, m42{}, m43{}, m44{}
    {}
    constexpr Matrix4(DoNotInitialize_Tag)
    {}
    constexpr Matrix4(Matrix4 const&) = default;
    constexpr Matrix4& operator=(Matrix4 const&) = default;

    constexpr Matrix4(T n11, T n12, T n13, T n14,
    ...
    constexpr explicit Matrix4(T const* arr, MatrixPolicies::InputOrder_RowMajor)
    ...
    constexpr explicit Matrix4(T const* arr, MatrixPolicies::InputOrder_ColumnMajor)
    ...
    [[nodiscard]] constexpr Vector4<T> row(std::size_t idx) const
    {
        auto const& m = *this;
        return Vector4<T>(m[idx*4], m[idx*4 + 1], m[idx*4 + 2], m[idx*4 + 3]);
    }

    [[nodiscard]] constexpr Vector4<T> column(std::size_t idx) const
    {
        auto const& m = *this;
    ...
    [[nodiscard]] friend constexpr auto operator<=>(Matrix4 const&, Matrix4 const&) = default;

    [[nodiscard]] friend constexpr Matrix4 operator+(Matrix4 const& lhs, Matrix4 const& rhs)
    {
        return Matrix4(lhs.m11 + rhs.m11, lhs.m12 + rhs.m12, lhs.m13 + rhs.m13, lhs.m14 + rhs.m14,
    ...
    [[nodiscard]] friend constexpr Matrix4 operator*(Matrix4 const& lhs, T f)
    {
        return Matrix4(lhs.m11 * f, lhs.m12 * f, lhs.m13 * f, lhs.m14 * f,
                       lhs.m21 * f, lhs.m22 * f, lhs.m23 * f, lhs.m24 * f,
                       lhs.m31 * f, lhs.m32 * f, lhs.m33 * f, lhs.m34 * f,
                       lhs.m41 * f, lhs.m42 * f, lhs.m43 * f, lhs.m44 * f);
    }

    [[nodiscard]] friend constexpr Matrix4 operator*(T f, Matrix4 const& rhs)
    {
        return Matrix4(f * rhs.m11, f * rhs.m12, f * rhs.m13, f * rhs.m14,
    ...
    [[nodiscard]] friend constexpr Matrix4 operator*(Matrix4 const& lhs, Matrix4 const& rhs)
    {
        return Matrix4(lhs.m11*rhs.m11 + lhs.m12*rhs.m21 + lhs.m13*rhs.m31 + lhs.m14*rhs.m41,
    ...
    [[nodiscard]] friend constexpr Vector4<T> operator*(Matrix4 const& m, Vector4<T> const& v)
    {
...
template<typename T>
[[nodiscard]] constexpr inline Matrix4<T> matrix_from_row_vectors(Vector4<T> const& r1, Vector4<T> const& r2,
...
template<typename T>
[[nodiscard]] constexpr inline Matrix4<T> matrix_from_column_vectors(Vector4<T> const& c1, Vector4<T> const& c2,
...
template<typename T>
[[nodiscard]] constexpr inline Matrix4<T> transpose(Matrix4<T> const& m)
{
...
template<typename T>
[[nodiscard]] constexpr inline T determinant(Matrix4<T> const& m)
{
...
template<typename T>
[[nodiscard]] constexpr inline Matrix4<T> adjugate(Matrix4<T> const& m)
{
    Vector3<T> const a(m.m11, m.m21, m.m31);
    Vector3<T> const b(m.m12, m.m22, m.m32);
    Vector3<T> const c(m.m13, m.m23, m.m33);
...
struct ScaledMatrix4
{
    ...
    constexpr ScaledMatrix4() = default;
    constexpr ScaledMatrix4(ScaledMatrix4 const&) = default;
    constexpr ScaledMatrix4& operator=(ScaledMatrix4 const&) = default;

    constexpr ScaledMatrix4(Matrix4<T> const& mat, T n_inverse_scale_factor)
        :m(mat), inverse_scale_factor(n_inverse_scale_factor)
    {}

    template<typename U>
    [[nodiscard]] constexpr Matrix4<U> evaluate() const
    {
        Matrix4<U> ret(m);
 ...
 */
template<typename T>
[[nodiscard]] constexpr inline ScaledMatrix4<T> inverse_scaled(Matrix4<T> const& m)
{
    Vector3<T> const a(m.m11, m.m21, m.m31);
    Vector3<T> const b(m.m12, m.m22, m.m32);
    Vector3<T> const c(m.m13, m.m23, m.m33);
...
[[nodiscard]] constexpr inline Matrix4<T> inverse(Matrix4<T> const& m)
{
...
template<typename T>
[[nodiscard]] constexpr inline Matrix4<T> identity4()
{
    using ::GHULBUS_MATH_NAMESPACE::traits::Constants;
